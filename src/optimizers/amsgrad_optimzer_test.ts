...
describeWithFlags('AMSGradOptimizer', ALL_ENVS, () => {
  it('basic', () => {
    const learningRate = .1;
    const beta1 = .8;
    const beta2 = .99;
    const optimizer = tf.train.amsgrad(learningRate, beta1, beta2);
    ...
    // Cost & 2 previous moments should be the only additional arrays.
    ...
    // ] = [.16, .64]
    // v = max [new_second_m, old_second_m] = [.16, .64]
    // x = [x - lr * m / sqrt(v)] = [1.8, 3.8]
    //
    expectArraysClose(x, [1.8, 3.8]);
    ...
    //
    expectArraysClose(x, [1.54657897, 3.54558067]);
